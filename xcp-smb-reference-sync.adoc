...
| <<sync -v>> | Increase debug verbosity.
...
| <<sync_smb_scan_acl,sync -fallback-user <fallback_user> >> 
...
| <<sync_smb_scan_acl,sync -fallback-group <fallback_group> >>
...
.Syntax
[source,cli]
xcp sync -mtimewindow <float> \\<IP address or hostname of SMB server>\copy_share2\\<IP address of SMB destination server>\source_share

.Show example
include::../_include/ref-example-smb-sync-mtimewindow.adoc[]

[[sync_smb_scan_acl]]
== sync -acl, -fallback-user, and -fallback-group
...
== sync -onlyacl, -fallback-user, and -fallback-group
...
.Syntax
[source,cli]
xcp sync -onlyacl -fallback-user <fallback_user> -fallback-group <fallback_group> \\<source_IP_address>\source_share \\<IP address of SMB destination server>\dest_share
...
.Syntax
[source,cli]
xcp sync -acl -aclverify yes -fallback-user <fallback_user> -fallback-group <fallback_group> \\<source_IP_address>\source_share \\<IP address of SMB destination server>\dest_share
