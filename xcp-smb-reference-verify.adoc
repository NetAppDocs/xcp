...
| <<verify -ads>> | The `verify` command with the `-ads` parameter checks for any alternate data streams on the source and destination and displays any differences.
...
| <<smb_verify_root,verify -root>> | Verify ACLs for the root directory.
| <<verify_smb_onlyacl, verify -onlyacl>> | Verify only the security information.
...
== verify -ads
Use the `-ads` parameter with the `verify` command to check if there are any alternate data streams on the source and destination and display any differences.
...
== verify -root
Use the `-root` parameter with the `verify` command to verify the ACLs for the root directory. 
