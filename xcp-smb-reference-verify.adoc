...
Use the `-onlyacl`, `-fallback-user` and `-fallback-group` parameters with the `sync` command to compare only the security information between the source and destination.
