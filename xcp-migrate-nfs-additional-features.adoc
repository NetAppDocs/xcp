...
NOTE: If you use the `XCP_CONFIG_DIR` or `XCP_LOG_DIR` environment variables to override the default config location or log location, the logdump command fails when run with an old migration or job ID.
