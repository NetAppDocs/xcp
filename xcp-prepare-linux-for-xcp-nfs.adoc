...
NOTE: You should store XCP catalogs separately. They should not be located on either the source or the destination NFS export directory. XCP maintains the reports in the catalog location specified during the initial setup. Before you run any operation using XCP, you must specify and update the location for storing the reports.
