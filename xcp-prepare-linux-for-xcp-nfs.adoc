...
* In ONTAP, you can use the diagnostic `-atime-update` option to preserve atime on source objects. This feature is only available in ONTAP and is helpful if you want to preserve atime in source objects while running XCP. This option is available to use with all commands that access source objects.
====
