...
| <<copy_acl4, copy -acl4>> | Processes NFSv4 access control lists (ACLs).
...
Use the `-nonames` parameter with the `copy` command to exclude user and group names for file listings or reports.
...
Use the `-acl4` parameter with the `copy` command to process NFSv4 ACLs. 
...
include::./_include/ref-example-nfs-copy-acl4.adoc[]
