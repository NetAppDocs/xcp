...
| <<scan_id, scan -id <catalog_name> >> | Specifies the catalog name of a previous copy or scan index.
| <<scan_match, scan -match <filter> >> | Only processes files and directories that match the filter.
| <<scan_fmt, scan -fmt <string_expression>> | Only processes files and directories that match the format.
| <<scan_du,scan -du>> | Summarizes the space usage of each directory, including subdirectories.
| <<scan_md5, scan -md5 <string_expression> >> | Generates checksums on the files and saves the checksums when indexing (default: false). 
| <<scan_depth,scan -depth <n> >> | Limits the search depth.
| <<scan_dircount, scan -dircount <n[k]> >> | Specifies the request size when reading directories.
| <<scan_edup, scan -edupe>> | Includes the dedupe estimate in reports.
| <<scan_bs, scan -bs <n[k]> >> | Specifies the read/write block size for scans that read data using `-md5` or `-edupe` (default: 64k).
| <<scan_parallel, scan -parallel <n> >> | Specifies the maximum number of concurrent batch processes (default: 7). 
