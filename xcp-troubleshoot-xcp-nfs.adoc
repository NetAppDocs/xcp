...
|Download the license from the link:https://xcp.netapp.com/[XCP site^] and copy to `/opt/NetApp/xFiles/xcp/`
...
|Mount the source and target on the XCP host
...
|Becuase a single file cannot be resumed, it is recommended to run the `xcp copy` command again for the file.
...
`For more details please refer XCP user guide.`
|As single file can not be synced, it is recommended to run the `xcp copy` command again for the file.
Any change in file leads to a complete copy of file. As a result, performance is not affected.
|===
