...
|If there is one or more colons in the NFS server share path name, use a double colon (::) to separate the NFS Server IP and NFS server share path instead of a single colon (:).
...
With the above steps complete, the SnapLock volume starts retaining the copied files.

NOTE: The retention time of a SnapLock volume is governed by the volume's default retention policy. Please check the volume retention settings before starting migration: link:https://docs.netapp.com/us-en/ontap/snaplock/set-retention-period-task.html[Set the retention time^]
|===
