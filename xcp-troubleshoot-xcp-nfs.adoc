...
|As a non-root user, you do not have the correct permissions to
access the file system. Check if you can access the file system and perform read write operations.
...
|The `sync` command is not supported in XCP for POSIX and HDFS connectors.
...
|`xcp: ERROR: mkdir: [Errno 13] Permission denied:`
