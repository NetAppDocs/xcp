...
.Syntax
[source,cli]
xcp>xcp scan -match <filter> \\<IP address or hostname of SMB server>\copy_share2
...
== scan -stats
Use the `-stats` parameter with the `scan` command to print tree statistics report formats.
...
== scan -html
Use the `-html` parameter with the `scan` command to retrieve tree statistics report formats.
...
== scan -csv
Use the `-csv` parameter with the `scan` command to retrieve tree statistics report formats.
...
== scan -l
Use the `-l` parameter with the `scan` command to retrieve a detailed file listing output.
