...
The `isync` command compares the source and destination and syncs the differences on the target without using the catalog index.
...
You can use `isync` with the `estimate` option to estimate the time it takes for the `isync` command to sync the incremental changes. 
