...
The `isync` command syncs changes on a target without indexing. 
...
You can use `isync` with the `estimate` option to estimate the time taken for incremental syncs.
